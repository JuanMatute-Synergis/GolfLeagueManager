...
# Import only Week 21 matchups (September 17) that failed previously
# This script will only process the Week 21 matchups from the CSV

API_BASE="http://localhost:5274/api"
CSV_FILE="/Users/juanmatute/Sources/GolfLeagueManager/schedule_with_names_corrected.csv"
...
create_matchup() {
    ...
    local json_payload=$(printf '{"playerAId":"%s","playerBId":"%s","weekId":"%s","scheduledDate":"2025-%s-%sT00:00:00Z"}' "$player1_id" "$player2_id" "$week_id" "$month" "$day")
...
echo "Starting Week 21 matchup import (September 17)..."
...
# Create temporary file to process only Week 21 matchups
temp_file=$(mktemp)
grep "^21,September 17," "$CSV_FILE" > "$temp_file"

echo "Found $(wc -l < "$temp_file") Week 21 matchups to process..."

# Process each Week 21 matchup
...
echo ""
echo "=== WEEK 21 MATCHUP IMPORT COMPLETED ==="
